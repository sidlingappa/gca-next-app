...
export function getAccess(role: Role, name: string, accessType: string): boolean {
    ...
    const readAccess = userRole?.permissions?.find(p => p.title === name + '-read');
    if (accessType === 'read' && readAccess) {
        return true;
    }
    const createAccess = userRole?.permissions?.find(p => p.title === name + '-create');
    if (accessType === 'create' && readAccess && createAccess) {
        return true;
    }
    const updateAccess = userRole?.permissions?.find(p => p.title === name + '-update');
    if (accessType === 'update' && readAccess && createAccess && updateAccess) {
        return true;
    }
    const deleteAccess = userRole?.permissions?.find(p => p.title === name + '-delete');
